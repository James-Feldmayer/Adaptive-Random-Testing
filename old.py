...
def transform(user_input_f):
    if type(re.match("def usable_case\(\S*\):", user_input_f)) == re.Match:
        ...
        try:
            ...
            try:
                illegal_variable = [7, 7, 6, 3, 13, 1,
                                    22, 18, 17]  # should take user input
...
buttonPrev = tk.Button(window, font=("Helvetica", 14),
                       text="Previous")  # , command=prevButton)
buttonNext = tk.Button(window, font=("Helvetica", 14),
                       text="Next")  # , command=nextButton)
